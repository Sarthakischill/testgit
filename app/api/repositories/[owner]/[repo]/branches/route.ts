...
export async function GET(
  ...
  try {
    ...
    const { data, status } = await octokit.request('GET /repos/{owner}/{repo}/branches', {
      owner,
      repo,
      per_page: perPage,
    });
    ...
    console.error(`Error getting branches for ${params.owner}/${params.repo}:`, error.status, error.response?.data || error.message);
    const ghStatus = error.status;
    const message = error.response?.data?.message || error.message || 'Failed to get branches';
